...
def update_one_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    order_db = sandwich.read_one(db, order_id=sandwich_id)
    if order_db is None:
        raise HTTPException(status_code=404, detail="User not found")
...
def delete_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    ...
    if sandwich is None:
        raise HTTPException(status_code=404, detail="User not found")
    return sandwich.sandwich(db=db, sandwich_id=sandwich_id)
...
def delete_one_resource(resource_id: int, db: Session = Depends(get_db)):
    ...
    if resource is None:
        raise HTTPException(status_code=404, detail="User not found")
    return resource.resource(db=db, resource_id=resource_id)
...
def delete_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    ...
    if recipe is None:
        raise HTTPException(status_code=404, detail="User not found")
    return recipe.recipe(db=db, recipe_id=recipe_id)
...
def read_one_order_detail(detail_id: int, db: Session = Depends(get_db)):
    orderdetail = order_details.read_one(db, detail_id=detail_id)
    if orderdetail is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orderdetail

@app.put("/order_details/{detail_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def update_one_order_detail(detail_id: int, orderdetail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    detail_db = order_details.read_one(db, detail_id=detail_id)
    if detail_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orderdetail.update(db=db, orderdetail=orderdetail, detail_id=detail_id)
...
def delete_one_order_detail(detail_id: int, db: Session = Depends(get_db)):
    ...
    if orderdetail is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orderdetail.orderdetail(db=db, detail_id=detail_id)